...
namespace Redbean
{
	public static partial class Extension
	{
		...
		public static UniTask CreateFirestore(this ISerializeModel model) =>
			FirebaseBootstrap.UserDB.SetAsync(model).AsUniTask();
		
		/// <summary>
		/// 파이어스토어 데이터 생성
		/// </summary>
		public static UniTask CreateFirestore(this ISerializeModel model, string key) =>
			FirebaseBootstrap.UserDB.SetAsync(new Dictionary<string, object> { { key, model } }).AsUniTask();
		
		/// <summary>
		/// 파이어스토어 데이터 업데이트
		/// </summary>
		public static UniTask UpdateFirestore(this ISerializeModel model, string key) =>
			FirebaseBootstrap.UserDB.UpdateAsync(key, model).AsUniTask();
